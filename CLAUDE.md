...
- **`api/`** — FastAPI routers: `playlists`, `live`, `vod`, `series`, `downloads`, `ws`, `settings`, `favorites`
  - `downloads` exposes per-item `pause`/`resume`/`retry`/`delete` plus `POST /api/downloads/bulk`. All five share the `_pause_one`/`_resume_one`/`_retry_one`/`_delete_one` helpers so each state rule is stated once; helpers raise `ValueError` for a rule violation and the caller decides whether that is a 400 or a per-item error. Bulk takes either `ids` (explicit selection) or `status` (everything in that state — this is how "retry all" works). Bulk retries are scheduled *after* the commit, otherwise the runner can read the row before `status=queued` is visible
